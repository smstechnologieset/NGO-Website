...
export default function SuccessStoriesSection() {
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-white via-paper to-paper border-b border-mutedBorder relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mb-12">
          {SUCCESS_STORIES.map((story) => {
            ...
            return (
              ...
              >
                <span className={`p-1 rounded-lg ${isSelected ? "text-accent bg-white/10" : "text-primary"}`}>
                  {story.icon}
                </span>
        ...
        <ScrollReveal animation="scale-up" duration={0.6} className="bg-white rounded-3xl border border-mutedBorder shadow-2xl overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
            {/* Image Side */}
            <div className="lg:col-span-6 relative min-h-[340px] lg:min-h-[460px] group overflow-hidden">
              <Image
              ...
              <div className="absolute top-6 left-6 z-10">
                <span className="px-4 py-2 rounded-full bg-white/90 backdrop-blur-md border border-mutedBorder text-primary font-bold text-xs shadow-md">
                  {activeStory.badge}
                </span>
              </div>
            ...
            <div className="lg:col-span-6 p-8 sm:p-10 lg:p-12 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-primary/10 text-primary font-semibold text-xs">
                ...
                <div className="space-y-2.5 pt-2">
                  ...
                  {activeStory.highlights.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 text-sm text-ink font-body font-medium">
                      <div className="w-5 h-5 rounded-full bg-accent/20 flex items-center justify-center text-accent font-bold text-xs flex-shrink-0">
                        ✓
                      </div>
                      <span>{item}</span>
                    </div>
              ...
              <div className="pt-6 border-t border-mutedBorder flex flex-wrap items-center justify-between gap-4">
                <button
                  onClick={() => setModalStory(activeStory)}
                  className="btn-shimmer px-6 py-3 rounded-full font-semibold bg-accent text-white hover:bg-accent-hover transition-all shadow-md text-sm inline-flex items-center gap-2"
                >
                ...
                <div className="text-xs text-ink/60 font-medium flex items-center gap-1.5">
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {SUCCESS_STORIES.map((story) => (
            <div
              key={story.id}
              onClick={() => setActiveId(story.id)}
              className={`glass-card glass-card-hover p-6 rounded-3xl cursor-pointer transition-all duration-300 border ${
                story.id === activeId
      ...
      {modalStory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-8 shadow-2xl border border-mutedBorder relative space-y-6 max-h-[90vh] overflow-y-auto">
            <button
              onClick={() => setModalStory(null)}
              className="absolute top-6 right-6 p-2 rounded-full bg-paper hover:bg-mutedBorder text-ink transition-colors"
            ...
            <div className="space-y-3">
              <span className="px-3.5 py-1 rounded-full bg-accent/20 text-ink font-semibold text-xs">
                {modalStory.badge}
              </span>
              <h3 className="font-display text-2xl font-bold text-ink">{modalStory.title}</h3>
              <p className="text-sm font-semibold text-primary">{modalStory.tagline}</p>
              <p className="text-sm text-ink/80 font-body leading-relaxed">{modalStory.description}</p>
            </div>

            <div className="p-4 rounded-xl bg-paper border border-mutedBorder space-y-2">
              <h5 className="text-xs font-bold uppercase text-ink/70">Key Program Components</h5>
            ...
            <div className="pt-2 text-right">
              <button
                onClick={() => setModalStory(null)}
                className="px-6 py-2.5 rounded-full bg-primary text-white font-semibold text-sm hover:bg-primary-hover transition-colors"
              >
                Close Story
              </button>
            </div>

...
export default function GalleryGridClient({
  ...
  if (!initialImages || initialImages.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-3xl border border-mutedBorder p-8 max-w-xl mx-auto shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {initialImages.map((image, index) => (
          <div
            ...
            className="group rounded-3xl overflow-hidden bg-white border border-mutedBorder/90 shadow-sm hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 cursor-pointer flex flex-col justify-between"
            tabIndex={0}
            role="button"
          ...
          >
            ...
            <div className="p-6 flex-1 flex flex-col justify-between">
              <div>
                <h2 className="font-display text-xl font-bold text-ink group-hover:text-primary transition-colors duration-300 line-clamp-1 mb-2">
              ...
              <div className="pt-4 mt-4 border-t border-mutedBorder/50 flex items-center justify-between text-xs text-ink/60">
                <span className="font-semibold text-primary">SCWOP Archive</span>
                <span className="group-hover:text-primary transition-colors font-medium">Click to Expand →</span>
              </div>
            </div>
          </div>
        ))}
